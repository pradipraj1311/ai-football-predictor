...
export const StandingsGrid: React.FC<{ standings: any[] }> = ({ standings }) => {
  ...
  return (
    <div className="flex flex-col gap-4 mt-4 shrink-0">
      {/* અહીં આપણે બધા જ ગ્રુપ્સ (Group A થી L) ને લૂપ (Loop) માં ફેરવી રહ્યા છીએ */}
      {standings.map((group, groupIdx) => (
        <div key={groupIdx} className="bg-[#0B1121] border border-white/5 rounded-xl flex flex-col overflow-hidden shadow-lg shrink-0">
